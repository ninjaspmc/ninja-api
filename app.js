...
var key = JSON.parse(fs.readFileSync("./database/keys.json"));
const users = JSON.parse(fs.readFileSync("./database/usuarios.json"));

async function listkeys(apikey, req) {
    var i4 = key.map(i => i?.apikey)?.indexOf(apikey);
    if (i4 >= 0) {
        key[i4].request -= 1;
        fs.writeFileSync("./database/keys.json", JSON.stringify(key, null, 2));
            const response = await axios.get("https://api.ipify.org?format=json");
            var IP = response.data.ip;
            var i3 = users.map(i => i.key).indexOf(apikey);
            if (i3 < 0 && !users.map(i => i.IP).includes(IP)) {
                users.push({ key: apikey, IP: [IP] });
                fs.writeFileSync("./database/usuarios.json", JSON.stringify(users, null, 2));
            } else if (i3 >= 0 && !users[i3]?.IP.includes(IP)) {
                users[i3].IP.push(IP);
                fs.writeFileSync("./database/usuarios.json", JSON.stringify(users, null, 2));
            }
        }
...
app.get('/list-keys', async (req, res, next) => {
...
listTokens = JSON.parse(fs.readFileSync(bla +'/database/keys.json').toString())
res.json(listTokens) 
})

app.get('/list-ip', async (req, res, next) => {
var senha = req.query.senha
if(!senha) return res.json({message: "A senha não foi preenchida, por segurança insira a senha que você criou."})
var senhaofc = "TioModz"
if(senha != senhaofc) return res.json({message: "A senha digitada está inválida, verifique por favor."})
...
app.get('/api/add-key', (req, res) => {
    ...
    var rq = req.query.rq;
    var senhaofc = "TioModz";
    if (senha !== senhaofc) return res.json({ resultado: "Senha inválida.." });
    ...
    if (key.map(i => i.apikey).includes(apikey)) {
        ...
        fs.writeFileSync("./database/keys.json", JSON.stringify(key));

        var ITC = key.map(i => i.apikey).indexOf(apikey);
        return res.json({
            resultado: `🔑 Chave ApiKey: ${apikey} Foi Adicionada ao Sistema com Sucesso!\n🚀 Numero de Requisições Disponíveis: ${key[ITC].request}`
        });
    }
});
...
const totalreq = JSON.parse(fs.readFileSync('./database/requests.json'))[0].totalreq;
